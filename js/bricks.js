...
function drawIt() {
	...
	function destroyBricks() {
		...
		if (!powerupActive) {
			...
			col = Math.floor(
				(WIDTH - x - r * 1.5 * (dx > 0 ? 1 : -1)) / colwidth
			);
		...
		if (rowmax > NROWS - 1) rowmax = NROWS - 1;
	...
	function drawPaddle() {
		if (downDown) {
			//paddel movment arows in omejevanje
	...
	function ballBounce() {
		if (y + dy > HEIGHT - r || y + dy < r) dy = -dy;

		if (x + dx > WIDTH - r) dx = -dx;
		else if ((x + dx - paddlew < r || x + dx - paddlew / 2 < r) && dx < 0) {
			//odboj od ploscka
			if (y > paddley + paddleh / shield && y < paddley + paddleh) {
				dy =
					6 *
					((y - (paddley + paddleh / shield + paddleh / 2.5)) /
						paddleh);
				dx = -dx;
				powerupActive = false;
				if (powerupPress) {
	...
	function powerupDestroy() {
		...
		else ctx.drawImage(rock, x - r, y - r, r * 2, r * 2);
	}

	function onKeyDown(evt) {
		if (evt.keyCode == 38 || evt.keyCode == 87) upDown = true;
		else if (evt.keyCode == 40 || evt.keyCode == 83) downDown = true;
	...
	function onKeyUp(evt) {
		...
		else if (evt.keyCode == 40 || evt.keyCode == 83) downDown = false;
	}
